...
export default function ReadingPage() {
  ...
  const handleSubmit = async (text: string, level: number) => {
    ...
    try {
      const { data, error: apiError } = await analysisAPI.reading({
        passage: text,
        hskLevel: level,
      });
      if (apiError) {
  ...
  return (
    <div className="max-w-[1000px] mx-auto px-4 md:px-8 py-8 md:py-12">
      ...
      <div className="mb-8 md:mb-10 text-center md:text-left flex flex-col md:flex-row md:items-end justify-between border-b border-black/5 pb-6">
        ...
        {loading && (
          <div className="mt-4 md:mt-0 flex items-center justify-center md:justify-start gap-2 text-[10px] font-mono text-blue-500 border border-blue-100 px-3 py-1.5 rounded bg-blue-50/50 shadow-sm w-fit mx-auto md:mx-0">
            <Skeleton className="w-2 h-2 rounded-full bg-blue-500" />
            AI_EXTRACTING...
          </div>
        )}
      </div>
        ...
        <div className="space-y-8 animate-in fade-in duration-500">
          ...
          <div className="grid lg:grid-cols-12 gap-8">
            <div className="lg:col-span-8 space-y-8">
              <Skeleton className="h-[200px] w-full bento-card" />
              <div className="space-y-4">
                <Skeleton className="h-6 w-48" />
                <Skeleton className="h-[120px] w-full bento-card" />
                <Skeleton className="h-[120px] w-full bento-card" />
              </div>
            </div>
            <div className="lg:col-span-4 space-y-8">
              <Skeleton className="h-[300px] w-full bento-card" />
              <div className="space-y-4">
                <Skeleton className="h-6 w-48" />
                <Skeleton className="h-[100px] w-full bento-card" />
                <Skeleton className="h-[100px] w-full bento-card" />
                <Skeleton className="h-[100px] w-full bento-card" />
              </div>
            </div>
        ...
        <div className="space-y-8">
          ...
          <div className="grid lg:grid-cols-12 gap-6 md:gap-8">
            <div className="lg:col-span-8 space-y-6 md:space-y-8">
              {/* Summary Section */}
              <div className="bento-card bg-white border-black/10 p-6 reveal-1">
            ...
            <div className="lg:col-span-4 space-y-6 md:space-y-8">
              ...
              {result.difficultWords && result.difficultWords.length > 0 && (
                <div className="bento-card bg-amber-50/50 border-amber-200 p-5 reveal-1">
